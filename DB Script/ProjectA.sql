...
@appointmenttime varchar(10)
as
insert into Appointments([PatientId],[VisitDate],[AppointmentTime],Specialization,DoctorName)
values(@patientid,@visitdate,@appointmenttime,@specialization,@doctor)
...
@spec varchar(50)
as
select concat(FirstName,LastName)as DoctorName,doc.Specialization
from DoctorInfo doc
join Appointments app
on AppointmentId = DoctorId
where (doc.Specialization like @spec) and (AppointmentTime in(VisitFrom,VisitTo))

create proc avail
@docid int
as
select VisitFrom,VisitTo
from DoctorInfo
where DoctorId=@docid

create proc updatespec
@specialization varchar(50),
@doctorname varchar(50)
as
update Appointments set Specialization=@specialization,DoctorName=@doctorname where AppointmentId=(select max(AppointmentId)from Appointments) 

updatespec Internalmedicine,Krishnakumar
select * from Appointments
Insappointment 1,"25/05/2022","8"

create proc showdoc
as
select concat(FirstName,LastName)as DoctorName,Specialization,VisitFrom,VisitTo
from DoctorInfo